...
output "WINDOWS-UtralVNC-vnc_connection" {
  value = "cmd /k cd %programfiles% & cd uvnc bvba & cd UltraVNC & vncviewer.exe -autoreconnect 30 -fullscreen -connect ${aws_eip.ip-test-env.public_ip}:5901 -password ${local.pass_rdp}"
}
