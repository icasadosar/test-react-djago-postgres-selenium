...
*/
  ...
  user_data = <<-EOF
        ...
        cat /tmp/.env-var.sh >> /root/.bash_profile
        cat /tmp/.env-var-git.sh >> /home/ec2-user/.bash_profile
        cat /tmp/.env-var-git.sh >> /root/.bash_profile
...
output "WINDOWS-UtralVNC-vnc_connection" {
  value = "cmd /k cd %programfiles% & cd uvnc bvba & cd UltraVNC & vncviewer.exe -autoreconnect 30 -connect ${aws_eip.ip-test-env.public_ip}:5901 -password ${local.pass_rdp}"
}
